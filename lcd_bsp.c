...
void lcd_lvgl_Init(void)
{
  ...
  lv_init();
  // RESTORED INTERNAL DMA RAM ALLOCATION (Fixes black screen)
  ...
  disp_drv.full_refresh = 0; // Disabled full refresh to fit in internal RAM
...
bool lvgl_port_lock(int timeout_ms) { return example_lvgl_lock(timeout_ms); }
void lvgl_port_unlock(void) { example_lvgl_unlock(); }
